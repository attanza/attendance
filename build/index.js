"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_rate_limit_1 = __importDefault(require("express-rate-limit"));
var helmet_1 = __importDefault(require("helmet"));
require("dotenv/config");
var workDay_1 = require("./libs/workDay");
var getCredentials_1 = require("./libs/getCredentials");
var checkInOffice_1 = require("./libs/checkInOffice");
var checkOutOffice_1 = require("./libs/checkOutOffice");
var express_1 = __importDefault(require("express"));
var limiter = (0, express_rate_limit_1.default)({
    windowMs: 1 * 60 * 1000,
    max: 10,
    standardHeaders: true,
    legacyHeaders: false, // Disable the `X-RateLimit-*` headers
});
var app = (0, express_1.default)();
var port = 10000;
app.disable('x-powered-by');
app.use((0, helmet_1.default)());
app.use(limiter);
app.get('/', function (_, res) { return res.send('Hello World!'); });
app.post('/check-in', function (_, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, niks, passwords, promises, i, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 6, , 7]);
                if (!(0, workDay_1.workDay)()) return [3 /*break*/, 4];
                _a = (0, getCredentials_1.getCredentials)(), niks = _a.niks, passwords = _a.passwords;
                promises = [];
                if (!(niks && niks.length > 0)) return [3 /*break*/, 2];
                for (i = 0; i < niks.length; i++) {
                    promises.push((0, checkInOffice_1.checkInOffice)(niks[i], passwords[i]));
                }
                // @ts-ignore
                return [4 /*yield*/, Promise.all[promises]];
            case 1:
                // @ts-ignore
                _b.sent();
                return [3 /*break*/, 3];
            case 2:
                res.send('no users');
                _b.label = 3;
            case 3:
                res.send('Thank you!');
                return [3 /*break*/, 5];
            case 4:
                res.send('not working day');
                _b.label = 5;
            case 5: return [3 /*break*/, 7];
            case 6:
                error_1 = _b.sent();
                console.log(error_1);
                res.status(500).send(error_1);
                return [3 /*break*/, 7];
            case 7: return [2 /*return*/];
        }
    });
}); });
app.post('/check-out', function (_, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, niks, passwords, promises, i, error_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 6, , 7]);
                if (!(0, workDay_1.workDay)()) return [3 /*break*/, 4];
                _a = (0, getCredentials_1.getCredentials)(), niks = _a.niks, passwords = _a.passwords;
                promises = [];
                if (!(niks && niks.length > 0)) return [3 /*break*/, 2];
                for (i = 0; i < niks.length; i++) {
                    promises.push((0, checkOutOffice_1.checkOutOffice)(niks[i], passwords[i]));
                }
                // @ts-ignore
                return [4 /*yield*/, Promise.all[promises]];
            case 1:
                // @ts-ignore
                _b.sent();
                return [3 /*break*/, 3];
            case 2:
                res.send('no users');
                _b.label = 3;
            case 3:
                res.send('Thank you!');
                return [3 /*break*/, 5];
            case 4:
                res.send('not working day');
                _b.label = 5;
            case 5: return [3 /*break*/, 7];
            case 6:
                error_2 = _b.sent();
                console.log(error_2);
                res.status(500).send(error_2);
                return [3 /*break*/, 7];
            case 7: return [2 /*return*/];
        }
    });
}); });
// custom 404
app.use(function (_, res, next) {
    res.status(404).send("Sorry can't find that!");
});
// custom error handler
app.use(function (err, _, res) {
    console.error(err.stack);
    res.status(500).send('Something broke!');
});
app.listen(port, function () {
    return console.log("Example app listening on http://localhost:".concat(port));
});
